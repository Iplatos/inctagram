...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3138
        NAMESPACE = "incubator-icta-trainee-uk"
        REGISTRY_HOSTNAME = "8blank8"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "inctagram-trainee"
        DEPLOYMENT_NAME = "inctagram-trainee-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'incubator-icta-trainee-uk') {
